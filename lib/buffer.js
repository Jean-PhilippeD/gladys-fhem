...
module.exports = {
    ...
    allLines.forEach(function (line)
    {
      ...
      if (line.length > 1)
      {
        if ((line.indexOf('humidity:') > -1) || (line.indexOf('temperature:') > -1) || (line.indexOf('brightness:') > -1) || (line.indexOf('motion:') > -1)) {
          array.push({protocol: line[0], id: line[1], value: line[3], type: type[line[2].replace(':','')].type});
        }
        if ((line.indexOf('channel0:') > -1) || (line.indexOf('channel1:') > -1)) {
          array.push({protocol: line[0], id: line[1], value: line[3], type: type.actuator.type});
        }

        if ((line.indexOf('closed') > -1) || (line.indexOf('open') > -1)) {
          array.push({protocol: line[0], id: line[1], value: line[2], type: type.contact.type});
        }

        if ((line.length === 3) && ((line.indexOf('AI') > -1) || (line.indexOf('A0') > -1) || (line.indexOf('B0') > -1) || (line.indexOf('BI') > -1))) {
          array.push({protocol: line[0], id: line[1], value: line[2], type: type.switchs.type});
        }
  ...
  verifyCommandReceived: function(data, expectedDevice, expectedAction) {
    return new Promise(function(resolve, reject) {
sails.log.warn(data);
sails.log.warn(expectedDevice);
sails.log.warn(expectedAction);
      var result = false;
      data.forEach(function (line) {
        line = line.trim().split(' ');
        if ((line.indexOf(expectedDevice) > -1) && (line.indexOf(expectedAction))) {
          // right action on device received
          var result = true;
        }
      });
      if(result) {
